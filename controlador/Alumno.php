...
$sesion = isset($_POST["sesion"]) ? limpiarCadena($_POST["sesion"]) : "";
...
switch ($_GET["op"]) {
    case 'guardaryeditar':
        ...
        if (empty($alumno_id)) {
            ...
            $rspta = $alumno->insertar($nombre, $apellidos, $documento_numero, $telefono, $carrera, $correo, $universidad,$turno,$sede,$curso,$sesion);
            ...
            $rspta = $alumno->editar($alumno_id, $nombre, $apellidos, $documento_numero, $telefono, $carrera, $correo, $universidad,$turno,$sede,$curso,$sesion);
            //devolvemos un mensaje segun el resultado de la operacion
    ...
    case 'buscar':
        // Llamamos al método buscar de la clase Alumno
        $rspta = $alumno->buscar($cedula);

        // Verificamos si la respuesta es válida
        if ($rspta === null || empty($rspta)) {
            // Si no hay datos, devolvemos una respuesta vacía o un mensaje de error
            echo json_encode(['error' => 'No se encontraron datos para esta cédula.']);
        } else {
            // Si hay datos, devolvemos la respuesta como objeto JSON
            echo json_encode($rspta);
        }
